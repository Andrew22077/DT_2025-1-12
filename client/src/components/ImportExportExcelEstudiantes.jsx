...
import { importExcelEstudiantes, exportEstudiantesExcel } from '../api/UserApi';

const ImportExportExcelEstudiantes = ({ onImportSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importExcelEstudiantes(selectedFile);
      
      if (result.created > 0) {
        setMessage(`✅ Importación exitosa: ${result.created} estudiantes creados de ${result.total_rows} filas procesadas`);
        ...
        setSelectedFile(null);
        document.getElementById('file-input-estudiantes').value = '';
        ...
        setMessage(`⚠️ No se crearon estudiantes. Revisa el formato del archivo.`);
        setMessageType('error');
  ...
  const handleExport = async () => {
    ...
    try {
      const result = await exportEstudiantesExcel();
      if (result.success) {
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-lg mb-6 border">
      ...
      <div className="mt-4 text-xs text-gray-500">
        ...
        <p>• Archivo con múltiples hojas: GRUPO VIRTUAL 1, EGP- 1A, EGP-1B, EGP -1C, EGP-2A, EGP- 2B, EGP-2C</p>
        <p>• Cada hoja debe tener secciones "PREMATRICULADOS" y "MATRICULADOS"</p>
        <p>• Columnas: No., DOCUMENTO, APELLIDOS Y NOMBRES, EMAIL INSTITUCIONAL</p>
        <p>• El grupo se toma automáticamente del nombre de la hoja y la fila 3</p>
...
export default ImportExportExcelEstudiantes;