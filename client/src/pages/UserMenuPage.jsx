...
export default function TeacherMenu() {
  return (
    ...
    >
      <div className="bg-white/90 p-8 sm:p-12 rounded-[40px] border-4 border-[#3399FF] shadow-2xl w-full max-w-[900px]">
        ...
        <div className="grid grid-cols-2 gap-5 sm:gap-6">
          {menuItems.map((item, index) => (
            <Link
              key={index}
              to={item.link}
              className="flex flex-col items-center justify-center min-h-[280px] border-2 border-[#99CCFF] rounded-2xl p-5 sm:p-6 bg-[#E6F2FF] hover:bg-[#3399FF] transition hover:scale-105 active:scale-95"
            >
              <div className="w-full h-36 sm:h-40 flex items-center justify-center mb-3 shrink-0">
                <img
                  src={item.image}
                  alt={item.label}
                  className="max-h-full max-w-full object-contain rounded-[16px] shadow-lg"
                />
              </div>
              <p className="text-base sm:text-lg font-bold text-[#003366] text-center">{item.label}</p>
            </Link>
          ))}
        </div>
      </div>
